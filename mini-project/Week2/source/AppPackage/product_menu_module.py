...
def new_product(created_product_str, prodlist):
    loop = 1
    while loop == 1:
...
def update_item(
    remove_product_str, replacement_product_str, list_output_func, prodlist, errorfunc
):
    temp_prod_list = prodlist
    loop = 1
    while loop == 1:
...
def del_item(remove_product_str, list_output_func, prodlist, error_func):
    loop = 1
    temp_prod_list = prodlist
    while loop == 1:
        list_output_func(temp_prod_list)
